...
.then(response => {
  ...
  if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    return api.sendMessage("I, Bashir's bot, have returned to you😉", event.threadID, () => api.sendMessage({ body: `${global.config.BOTNAME} CONNECTED«\n\nAssalamualaykum☘️
<------------------------------>  
...
DEVELOPER :Bashir Ahamed 

🟣Facebook Account Link: 

https://www.facebook.com/Mr.Infinity.11.19?mibextid=ZbWKwL

🔵WHATSAPP NUMBER: wa.me/+8801977147238

🟢SUPPORT EMAIL: www.bashirahamed@gmail.com`, attachment: fs.createReadStream(gifPath)}, threadID));
  }})
.catch(error => {
    console.error(error);
});
    ...
    try {
      if (!fs.existsSync(__dirname + `/Bashir/font/Semi.ttf`)) {
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        let pathImg = __dirname + `/Bashir/join/${o}.png`;
        let pathAva = __dirname + `/Bashir/join/avt.png`;
